...
bool initialize()
{
	vector<glm::vec4> ob_vertices;
	vector<glm::vec3> ob_normals;
	vector<GLushort> ob_elements;
	load_obj("board.obj", ob_vertices, ob_normals, ob_elements);
	// Create a Vertex Buffer object to store this vertex info on the GPU
	glGenBuffers(1, &vbo_geometry);
	glBindBuffer(GL_ARRAY_BUFFER, vbo_geometry);
	glBufferData(GL_ARRAY_BUFFER, sizeof(ob_vertices), ob_vertices, GL_STATIC_DRAW);
	//--Geometry done
